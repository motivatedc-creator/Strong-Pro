...
test.describe('starter routines', () => {
  test('adding a starter routine creates a real, editable routine that survives a reload', async ({
    page,
  }) => {
    await freshApp(page);

    await page.goto('/templates');
    await expect(page.getByText('No custom routines yet')).toBeVisible();
    ...
    await sheet.getByRole('button', { name: 'Add to my routines' }).first().click();
    await expect(page.getByText('"Full Body Strength" added to your routines.')).toBeVisible();
    await sheet.getByRole('button', { name: 'Close' }).click();

    // It's a real routine: listed, startable, editable, and it survives a reload.
